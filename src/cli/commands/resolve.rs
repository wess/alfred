...
pub fn run(target_file: Option<String>) -> Result<()> {
    ...
    ui::heading(format!("Found {} conflicted file(s):", status.conflicts.len()));
    ...
    for file in &files_to_resolve {
        ...
        let conflict_info = match git::get_conflict_info(file) {
            Ok(info) => info,
            Err(e) => {
                ui::warn(format!("Cannot extract conflict versions for {}: {}", file, e));
                continue;
            }
        };
        ...
        let action = ui::select("What would you like to do?", &options)
            .unwrap_or("Skip this file");
    ...
    if remaining.conflicts.is_empty() {
        ...
        ui::warn(format!("{} conflict(s) remaining", remaining.conflicts.len()));
    }
