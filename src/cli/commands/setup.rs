...
pub async fn run() -> Result<()> {
    ...
    println!();
    println!("Alfred uses a local AI model for git assistance - no API keys or subscriptions needed.");
    println!();
    ...
    let existing_models: Vec<String> = fs::read_dir(&models_dir)?
        ...
        .filter(|entry| {
            entry.path().extension().map(|e| e == "gguf").unwrap_or(false)
        })
    ...
    for (i, model) in MODELS.iter().enumerate() {
        println!("  {}. {} ({})", (i + 1).to_string().cyan(), model.name, model.size);
    }
    println!();
